(() => {
  ...
  const tokenKey = "lord_token"; // sessionStorage (tab yopilsa yo'qoladi)
  ...
  const setToken = (t) => sessionStorage.setItem(tokenKey, t);
  ...
  async function loadMe() {
    ...
    if (me.role === "ADMIN") adminBlock.classList.remove("hidden");
  ...
  async function loadHistory() {
    ...
    rows.forEach((r) => {
      ...
      row.innerHTML = `
        <div>${r.direction}</div>
        <div class="mono">${r.counterparty}</div>
  ...
  sendForm.onsubmit = async (e) => {
    ...
    try {
      ...
      const out = await api("/api/v1/tx/transfer", {
        method: "POST",
        body: JSON.stringify({ receiver_address, amount_usdt }),
      });
  ...
  explorerForm.onsubmit = async (e) => {
    e.preventDefault();
    explorerOut.textContent = "Checking…";
    try {
      const h = txHash.value.trim();
      const out = await api(`/api/v1/explorer/tx/${encodeURIComponent(h)}`);
  ...
  // Admin tabs (prints JSON, simple & safe)
  document.querySelectorAll(".admin-tabs button").forEach((b) => {
    b.onclick = async () => {
      try {
        const k = b.getAttribute("data-a");
        adminOut.textContent = "Loading…";
        if (k === "users") adminOut.textContent = JSON.stringify(await api("/api/v1/admin/users"), null, 2);
        if (k === "txs") adminOut.textContent = JSON.stringify(await api("/api/v1/admin/transactions"), null, 2);
        if (k === "logs") adminOut.textContent = JSON.stringify(await api("/api/v1/admin/logs"), null, 2);
      } catch (err) {
